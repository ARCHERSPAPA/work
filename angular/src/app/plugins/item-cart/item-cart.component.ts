...
export class ItemCartComponent implements OnInit {
    ...
    public clickSelectAll: boolean = false;
    //全选时的加载情况
    public loadingModuleAll: boolean = true;

    public bufferName: string;

    //区别展示与否
    public toggleType: string = "down";
    ...
    ngOnInit() {
        ...
        let buf = this.buffer.getBuffer(this.bufferName);
        if (buf) {
            let sd = this.buffer.getBuffer(this.bufferName);
            ...
            this.request.doPost({
                ...
                success: (res => {
                    if (res && res.code === 200) {
                        ...
                        // }
                        if (this.category) {
                            let findCategory = this.categoryList.filter(cate => cate === this.category);
    ...
    //获取终点坐标
    getEndPoint() {
        let point = this.el.nativeElement.querySelector("#count");
        return {
    ...
    getVersionName(id): string {
        if (id && this.changeOptions && this.changeOptions.length > 0) {
            let v = this.changeOptions.filter(item => item.versionId === id);
    ...
    exist(id): boolean {
        if (this.selectModules && this.selectModules.length) {
            let m = this.selectModules.filter(m => m.id === id);
            return m && m.length > 0;
        }
        return false;
    }
    ...
    moduleSize(id: number) {
        if (this.selectModules && this.selectModules.length > 0) {
            let size = this.selectModules.filter(m => m.id === id);
