...
export class EngineesComponent implements OnInit {
        ...
        for (let so of source) {
            if (that.getTotalInType(so.type)) {
                if (so.infoMaps && so.infoMaps.length > 0) {
                    ...
                    let checkedAll = so.infoMaps.every(sin => sin.checked === true);
                    let unCheckedAll = so.infoMaps.every(sin => (!sin.checked && !sin["indeterminate"]));
    ...
    middleChange(bool: boolean, data: any, type: number) {
        if (data && data.infos && data.infos.length > 0) {
            ...
            let allChecked = data.infos.every(sin => sin.engineerYn === true);
            let allUnChecked = data.infos.every(sin => !sin.engineerYn);
        ...
        if (source && source.length > 0) {
            for (let so of source) {
                if (so.type === type && so.infoMaps && so.infoMaps.length > 0) {
                    ...
                    // });
                    let allChecked = so.infoMaps.every(sin => sin.checked === true);
                    let allUnChecked = so.infoMaps.every(sin => (!sin.checked && !sin["indeterminate"]));
