...
export class MasterAuxiliaryComponent implements OnInit {
    ...
    public model: string;
    //加载
    public isConfirmLoading: string;
    ...
    //销售弹窗
    public sellVisible: boolean = false;
   ...
   //销售单价的提交
   modelUnitChange(material) {
    let param = {};
    if (material) {
        if (material['sellingUnit'] && material['sellingUnit'].trim()) {
            param['materialId'] = Array.of(material['id']);
            param['sellingUnit'] = material['sellingUnit'].trim();
        } else {
            this.warn.onMsgError('请填写1~30位字符');
            this.changeData();
            return;
        }
    } else {
        ...
        param['materialId'] = ids;
        param['sellingUnit'] = this.sellUnit;
    }
    this.req.doPost({
        url: 'masterSellSave',
        data: param,
