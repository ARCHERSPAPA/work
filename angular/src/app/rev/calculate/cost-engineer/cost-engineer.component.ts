...
export class CostEngineerComponent implements OnInit {
    ...
    submit(type) {
        ...
        if(this.modifyForm.valid){
            if (this.modifyForm.value.percentage) {
                this.percentage = +this.modifyForm.value.percentage.toFixed(2);
            }
            if (this.modifyForm.value.addPercentage) {
                this.addPercentage = +this.modifyForm.value.addPercentage.toFixed(2);
            }
                ...
                if (this.lock) {
                    ...
                    me = setTimeout(() => {
                        if (type === 0) {
                            ...
                            this.loadRatio(this.modifyForm.value.designPrice, this.id, type);
                        }
                        this.lock = false;
                    }, 1000);
                }
