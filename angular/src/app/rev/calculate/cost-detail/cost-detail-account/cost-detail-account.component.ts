...
import {atob, subItemName} from "../../../../model/methods";
...
export class CostDetailAccountComponent implements OnInit {
    ...
    moveItem(item: any, menu: any, ...args) {
        ...
        console.log("this store data",this.storeData);
    ...
    getTotalByType(infos, num, price) {
        ...
        if (infos && infos.length > 0) {
            total = infos.reduce((total, info) => {
                return total + (this.computedSingleTotal(info["" + num + ""], info["" + price + ""]));
            }, 0)
        }
        return total;
    }
     ...
     */
    showSendByState() {
        return this.showItemByTypes([0, 1, 2], this.state) && this.isSuperUser();
    }
    ...
    editInfoByState() {
        if(this.isSuperUser()){
            return this.showItemByTypes([3, 4], this.state)
        }
        return true;
    }
    ...
    disabledOperateByState(){
        return !this.showItemByTypes([3, 4], this.state) && this.isSuperUser();
    }
