...
export class SupplierMaterialEditComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (!this.newMaterial) {         //异步获取回显数据
            ...
            this.preLIst.push(this.modalList)
    ...
    //2.3.0中应后端需要只能在开始有图片的时候才能设置默认图片
    checkSet(material) {
        if (material.oldImage && material.id) {
            return true
        } else if (this.newMaterial) {
            return true
        } else {
            return false;
        }
    ...
    uploadImg(i) {
        ...
        modalRef.result.then((result) => {
            that.materialSaleList[i].fileName = result.fileName.substr(0, 4) + '...';
    ...
    copy(item, i) {
        ...
        console.log(this.materialSaleList,this.currentIndex,this.materialSaleList[this.currentIndex])
    ...
    edit(i) {
        ...
        this.preLIst.forEach(v => {
            if (v['key'] == this.materialSaleList[i]['key']) {
                v = this.resData(this.materialSaleList[i]);
            }
        });
    }
    ...
    cancel(i) {
        this.preLIst.forEach(v => {
            if (v['key'] == this.materialSaleList[i]['key']) {
                this.materialSaleList[i] = this.resData(v);
                this.materialSaleList[i]['edit'] = false;
            }
        });
    }
    ...
    del(i, material) {
        if (material.id) {
            ...
            this.currentIndex -= 1;
            console.log(this.preLIst,this.materialSaleList)
            this.preLIst.forEach((v, index) => {
     
                if ((v['key'] == this.materialSaleList[i]['key']) && !this.materialSaleList[i]['edit']) {
                    this.preLIst.splice(index, 1);
                    this.materialSaleList.splice(i, 1);
                    return;
                }
