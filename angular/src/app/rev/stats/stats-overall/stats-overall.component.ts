...
import {getStateName, getTypeName,getStatsTypeName} from '../../../model/methods';
...
export class StatsOverallComponent implements OnInit {
	...
	public ismouseenter = false; //防止鼠标移入一直触发
	public today: Date = new Date();
	public lineIdx = '-1'; //鼠标移入移出只显示当前折线图索引
		...
		that.request.doPost({
            ...
            success: (res => {
                ...
                if (res && res.code == 200) {
					...
					this.data = res.data;
					// 数据求和
					const customerCount = res.data.reduce(function (total, currentValue) {
						return total + currentValue['客户数'];
					}, 0);
					const offerCount = res.data.reduce(function (total, currentValue) {
						return total + currentValue['报价数'];
					}, 0);
					const signBillCount = res.data.reduce(function (total, currentValue) {
					...
					const withdrawalCount = res.data.reduce(function (total, currentValue) {
					...
					const statistics = [customerCount, offerCount, signBillCount, signBillAmount, withdrawalCount, withdrawalAmount];
  	...
  	chartData() {
		...
		this.chart.interact('slider', {
			...
			onChange(ev) {
				...
				let endDate = new Date(end); */
				const endDate = new Date(endValue + 24 * 60 * 60 * 1000 - 1).getTime();
	...
	clearView() {
		...
		const childs = slider.childNodes;
			...
			success: res => {
				if (res && res.code == 200) {
					...
					excelData.forEach(item => {
						...
						const date = new Date(item.billTime);
						const seperator1 = '-';
						const year = date.getFullYear();
						let month: any = date.getMonth() + 1;
						let strDate: any = date.getDate();
