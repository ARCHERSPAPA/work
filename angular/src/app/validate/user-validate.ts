...
export function ValidateAccount(fc: FormControl) {
  const acc = Reg.ALPHA_NUM_UNDERLINE_ZH;
  if (!fc.value) { return true; } else { return acc.test(fc.value) ? null : {
    account: {valid: false}
  };
  }
}
...
export function ValidatePassword(fc: FormControl) {
  const pwd = Reg.ALPHA_AND_NUM_UNDERLINE;
...
export function ValidatePhone(fc: FormControl) {
  ...
  if (!fc.value) { return true; } else { return phone.test(fc.value) ? null : {
    phone: {valid: false}
  };
  }
}
...
export function ValidatePrice(fc: FormControl) {
    const reg = Reg.NUM_MONEY_DECIMAL;
    if (!fc.value) { return true; }
 ...
 * @constructor
 */
export function ValidateNumToInt(fc: FormControl) {
    if (!fc.value) { return true; }
    return Reg.NUM_INT.test(fc.value) ? null : {
        number: {valid: false}
    };
}
