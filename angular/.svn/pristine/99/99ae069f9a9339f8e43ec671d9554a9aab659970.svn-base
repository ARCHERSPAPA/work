...
import {btoa, changeToDecimal, equalToSame, showMakeBtnByState, toInteger} from "../../../model/methods";
...
export class MakingsComponent implements OnInit {
    ...
    public markVisible: boolean = false;
    public markItem: any;
    //备注信息
    ...
    setDefaultMake(){
        let mat;
        let data = OFFER_DEFAULT_DATA.filter(d => {
    ...
    copyItem(e: any, data: any, type: number, ...args) {
        ...
        e.preventDefault();
        if (type === 1) {
            let infos = this.material.infoMaps[args[0]].infos;
            ...
            if (data.name) {
                ...
                //     data: {
                //         quoteId: this.cid,
                //         infoBranchName: data.name,
                //         type: Default.OFFER_ITEM.ITEM_5
                //     },
                //     success: (res => {
                //         if (res && res.code == 200) {
                //             this.warn.onMsgSuccess(res.msg || Messages.SUCCESS.DATA);
                //             this.copyBranch(data.infoBranchId, {
                //                 infoBranchId: res.data.infoBranchId,
                //                 name: res.data.infoBranchName
                //             });
                //         } else {
                ...
                //     data:{
                //         quoteId: this.cid,
                //         copyId: data.infoBranchId,
                //         type: type
                //     },
                //     success:(res =>{
                //         if(res && res.code == 200){
                //             this.material = res.data?res.data[0]:null;
                //         }else{
                //             this.warn.onMsgError(res.msg || Messages.FAIL.DATA);
                //         }
                //     })
            ...
            // this.material.infoMaps.splice(args[0],0,data);
        }
    ...
    copyBranch(cd:any){
        if(this.material && this.material.infoMaps && this.material.infoMaps.length > 0){
            let infos = this.material.infoMaps.filter(map => map.id === cd.id);
            let index = this.material.infoMaps.findIndex(map => map.id === cd.id);
            let copyInfos = {id: this.getRenderId("c"),name:cd.name,infos:infos[0].infos};
    ...
    delItem(i: number, t: number) {
        let infos = this.material.infoMaps[i].infos;
        infos.splice(t, 1);
    }
    ...
    saveMaterial(e: any) {
        e.stopPropagation();
        e.preventDefault();
        let map = this.getParamsByList(this.material);
        if (this.cid) {
            this.request.doPost({
                url: "submitMaterial",
                data: {
    ...
    //显示每栏的提示按钮
    showBtnByState() {
        return showMakeBtnByState(this.baseQuote) && equalToSame(this.user.getPhone(), this.designers) && this.state !== Default.STATE.ITEM_6 && this.baseQuote.retreat === 0;
    }
    ...
    modifyBranchOk() {
        ...
        if (that.branchEdit && that.branchId) {
            let branch = this.material.infoMaps.filter(map => map.id === that.branchId);
    ...
    addMaterialInfos(bid: string, infos: any) {
        if (this.material.infoMaps && this.material.infoMaps.length > 0) {
            let infoMap = this.material.infoMaps.filter(map => map.id === bid);
            if (infoMap && infoMap.length === 1) {
                infoMap[0]["expand"] = true;
                let infoArray = JSON.parse(infos);
