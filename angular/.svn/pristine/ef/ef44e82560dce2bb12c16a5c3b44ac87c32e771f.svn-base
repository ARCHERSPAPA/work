...
export class ItemComponent implements OnInit {
    ...
    fillCount() {
        if (this.itemForm.get("univalent") && this.itemForm.get("num")) {
            let univalent = this.itemForm.get("univalent").valid ? this.itemForm.get("univalent").value : "";
            let num = this.itemForm.get("num").valid ? this.itemForm.get("num").value : "";
    ...
    modify(e:any) {
        ...
        if (this.aid && this.itemForm.valid) {
            let params = this.itemForm.value;
    ...
    changeChildOptions(...args) {
        ...
        if (that.configBase && that.configBase.length > 0) {
            ...
            let cps = that.configBase.filter(tc => tc.type === that.selectType);
            if(cps && cps.length > 0){
                console.log(cps);
