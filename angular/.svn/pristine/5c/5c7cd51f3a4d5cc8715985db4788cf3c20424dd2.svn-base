...
export class ItemPublishComponent implements OnInit {
     ...
     * @param design
     */
    selectDesign(design: any, index: number) {
        this.staffList.forEach(item => {
            if (design.id === item.id) {
                if (item["checked"] == false && this.designs) {
                    this.designs.forEach((items, i) => {
                        if (items['id'] === design.id) {
                            this.designs.splice(i, 1, '');
                            this.fixed();
                        }
                    })
                } else {
                    this.designs.splice(index, 0, item)
