    ...
    self.request.doPost({
      ...
      success:(res =>{
        ...
        if(res && res.code == 200){
          ...
          self.id = self.activatedRoute.snapshot.queryParams["id"];
          console.log( self.activatedRoute.snapshot.params)
          if(!isNaN(Number(self.id))){
            ...
            if(self.cookieService.get("depart")){
              let arrCopy:any = self.cookieService.get("depart").split(',');
