...
export class SettleDetailAttendComponent implements OnInit {
    ...
    //交互
    public selectMonth: number = 0;
    public selectDay: number = 0;
    ...
    getLocal(select, type) {
        // console.log(select);
        let modal = this.modalService.open(LocationComponent, {
            centered: true,
            keyboard: true
        });
        modal.componentInstance.title = "查看当前地理位置";
        let point = {lng: 104.065971, lat: 30.657154};
        if (type === "first") {
    ...
    //审核弹窗通过与不通过
    confirm(ct: number) {
        let url = ct ? "agreeLabourExpenses" : "rejectLabourExpenses";
        this.req.doPost({
            url: url,
    ...
    justColorByDate(date: Date) {
        let that = this;
        let month = that.getMonth(date);
        let day = that.getDate(date);
        if (that.data && that.data.length > 0) {
            for (let d of that.data) {
                if (d["month"] === month && d["day"] === day) {
                    if (d.firstCheck && d.lastCheck) return "all";

                    else if (d.firstCheck && !d.lastCheck) return "morning";

                    else if (!d.firstCheck && d.lastCheck) return "night";
