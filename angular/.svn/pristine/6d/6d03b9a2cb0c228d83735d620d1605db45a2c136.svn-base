...
export class SupplierMaterialInfoComponent implements OnInit {
  ...
  handleFileChange(e) {
    ...
    httpParams.set('supplierId', this.id);
    this.httpOptions['params'] = { 'supplierId': this.id + '' }
  ...
  handleSwitch(e) {
    this.pageType = e;
    this.pageNo = Default.PAGE.PAGE_NO;
    this.router.navigate(['./'], { queryParams: { pageType: this.pageType, id: btoa(this.id), name: this.title }, relativeTo: this.activatedRoute });
    this.changeData();
  }
  //审核撤回
  apply(id) {
