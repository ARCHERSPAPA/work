...
export class InputDecimalDirective {
    ...
    @HostListener('change', ['$event']) onChange(e) {
        ...
        if (this.bc == null) {
            if (this.role === 1) {
                let formula = this.el.nativeElement.value;
                let num = this.arith.computed(formula);
                let params = {
                    id: this.infoId,
                    num: num
                };
                ...
                if(Reg.OPERATOR_SYMBOL.test(formula)){
                    let cformula = this.el.nativeElement.value;
                        ...
                        let result = (1*Math.pow(10,w))/(p*Math.pow(10,w));
                        return "*"+result;
                    })
                    let num = this.arith.computed(cformula);
