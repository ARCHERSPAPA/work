...
export class StatsAttendanceComponent implements OnInit {
  ...
  //日历
  public currentDate = new Date();  //当前选中时间
  public Date = new Date();   //日历展示时间
  public DateInfo;
  public supervisor = [];
  public DateLoading = true; 
  public selectSupervisor:number;//选则的人员ID
  public attendTimeRange: any = {
    startTime: Date.parse(new Date().getFullYear() + ''), //初始起始可选时间（默认当年到当天）
    endTime: Date.now()
  };
  ...
  public headInfo = { avatar: 'https://qiniu.madrock.com.cn/rev/imgs/435467b6-6b9e-2a36-a51b-bde1d77a31ce.png', name: '请在下方选择监理', positionName: '暂无职位' };
  ...
  //表单
  public statList;
  public Loading = true;              //初始表格数据
  public dataList = {
    monthly: { totalSiteCount: 0, startSiteCount: '', finishSiteCount: '', postCount: '', uniquePostCount: '' },
    daily: { totalSiteCount: '', postCount: '', uniquePostCount: '' }, dailyDetail: []
  };
  ...
  renderMark() {
    ...
    curArr.forEach((v, i) => {
      ...
      if (i > 0 && this.nextPath.latitude == v.latitude && this.nextPath.longitude == v.longitude) {
        offset = new AMap.Pixel(-30 + i * 5, -40 + i * 5)
      } else {
        offset = new AMap.Pixel(0, 0)
      }
      this.nextPath = v;
      click = v['k'] ? true : false;            //点击事件Marker
...
`
  ...
  renderData(data) {
    if (data.monthly) {
      this.dataList.monthly = data.monthly;
    }
    if (data.daily) {
      this.dataList.daily = data.daily;
    }
    if (data.dailyDetail) {
      this.dataList.dailyDetail = data.dailyDetail;
    }
    this.Loading = false;
    if (data && data.employee) {
      this.headInfo['positionName'] = data.employee.positionName ? data.employee.positionName : '--';
      this.headInfo['name'] = data.employee.name ? data.employee.name : '--';
      this.headInfo['avatar'] = data.employee.headImg ? data.employee.headImg : 'https://qiniu.madrock.com.cn/rev/imgs/435467b6-6b9e-2a36-a51b-bde1d77a31ce.png';
    } else {
