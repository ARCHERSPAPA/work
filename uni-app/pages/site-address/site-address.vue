<template>
  <view class="site-addr">
    ...
    <view v-if="siteList && siteList.length > 0" class="site-addr-box">
      <view v-for="(site,index) of siteList" :key="index" class="site-addr-box-item" @click="select(site)">
        ...
        <view class="item-info">
          ...
          <view v-if="site && site.distance == 0" class="item-info-icon"><uni-icons type="yixuan" color="#FF8800" size="40" /></view>
        </view>

      </view>
    </view>
    <view v-else class="site-addr-box">
      <mkb-empty :text="'暂无查询地理信息'" />
    </view>
  </view>
</template>
...
import mkbSearchBar from '../../components/mkb-search-bar/mkb-search-bar'
export default {
  components: {
    mkbSearchBar
  },
