		...
		<view v-show="!showSearchBar">
			<view class="mkb-search-head">
				<view class="mkb-search-head-select">
					<search-select class="price-result-select-single" :theme-color="headColor" :menu-list="headMenuList" :select-list="headSelectItems"
					 :close-select="closeSelect" @headResult="headResult" @selectPopup="selectPopup" />
				</view>
				<view class="mkb-search-head-input" @tap="closeSearchBar">
					<uni-search-bar ref="searchResultRef" class="mkb-search-head-input" :radius="28.98" :placeholder="searchBar.placeholder"
					 :clear-button="'none'" cancel-button="none" />
				</view>
			</view>
			<!-- 筛选条件 -->
			<view class="mkb-content">
				<sl-filter ref="slFilter" class="mkb-content-single" :theme-color="themeColor" :menu-list="menuList" 
				:select-sort="selectSort" :select-list="selectItems" :close-popup="closePopup" 
				@selectItem="selectItem" @sortResult="sortResult" @screen="screen"
			...
			<view>
				...
				<view v-else>
					<mkb-empty :text="'暂无查询数据'" v-if="loading"/>
				</view>
			</view>
		</view>
